...
// to properties (zero, one or many) through the join table.
...
export async function fetchInsurancePolicies() {
  ...
  if (error) throw error
  // Flatten the joined property list onto each policy as `properties`,
  // and keep only the property objects (not the join rows).
  return (data || []).map(p => ({
    ...p,
...
export async function updateInsurancePolicy(id, updates, propertyIds) {
  ...
  if (Array.isArray(propertyIds)) {
    // Replace strategy: delete then insert. Simpler than diffing.
...
/**
 * Soft-delete a policy. Property links stay (via FK CASCADE on hard delete
 * only — soft delete keeps them but the policy is filtered out of fetches).
 */
 ...
 *
 * Returns the new policy id. Caller should open the modal to let the user
 * adjust the new premium and any other details before saving.
 */
export async function renewInsurancePolicy(oldPolicyId, overrides = {}) {
  const userId = await uid()
  // Read the old policy + its property links
  ...
  // Default new dates: start = old expiry, expiry = old expiry + 1 year
  ...
  const newPolicy = {
    ...
    premium:          old.premium, // user will likely adjust
...
// ── PORTFOLIO INSIGHTS ─────────────────────────────────────────────────────
// Fetch the latest AI-generated insights for the current user. Returns null
// if they've never generated any.
...
// ── TENANT REFERENCING ─────────────────────────────────────────────────────
// Pre-partnership stage: orders are stored locally and surfaced to the user
// as "Coming soon". Once a partner contract is in place we'll add an edge
// function that POSTs to the partner API and updates the row's status and
// result fields.
...
// ── OPEN BANKING ───────────────────────────────────────────────────────────
// Pre-partnership stage: bank-connection rows store user expressions of
// interest. Real OAuth handoff to TrueLayer / Plaid / GoCardless comes
// when we sign a partner contract. Schema and policies are already in
// place via 2026-05-19_bank_connections.sql.
