...
//
// Escalation (added 2026-09-07 after six unactioned AMBER nights): the email
// subject carries the streak and the first item needing attention, so a
// second consecutive AMBER or any RED reads as "ACTION NEEDED" in an inbox
// rather than as one more routine digest. AUDIT_ESCALATION_TO, when set, is
// copied in on those nights only.
//
// Env vars required:
...
//   AUDIT_ESCALATION_TO          (optional second recipient on escalated nights)
...
//
// Source of record: this file. Deployed v5 was pulled into the repo on
// 2026-09-07 (PR #78 had held an older copy since 10 August).
...
const ESCALATION_TO  = Deno.env.get('AUDIT_ESCALATION_TO') || ''
...
// ── Client errors (browser crashes reported by the app) ────────────────
// Reads public.client_errors, written by the app's ErrorBoundary and window
// error handlers (added 2026-09-07). Absent table = check is skipped, so the
// audit keeps working on a database that predates it.
async function clientErrorCheck(admin: any): Promise<Check | null> {
  try {
    const since = new Date(Date.now() - 86400000).toISOString()
    const { data, error } = await admin.from('client_errors')
      .select('message, page, created_at').gte('created_at', since).order('created_at', { ascending: false }).limit(200)
    if (error) {
      if (/does not exist|relation/.test(error.message)) return null
      return { name: 'client_errors', group: 'platform', label: 'App errors (24h)', status: 'warn', detail: `query failed: ${error.message.slice(0, 100)}` }
    }
    const rows = data || []
    if (!rows.length) return { name: 'client_errors', group: 'platform', label: 'App errors (24h)', status: 'ok', detail: 'none reported' }
    const byMsg = new Map<string, number>()
    for (const r of rows) byMsg.set(String(r.message).slice(0, 80), (byMsg.get(String(r.message).slice(0, 80)) || 0) + 1)
    const top = [...byMsg.entries()].sort((a, b) => b[1] - a[1]).slice(0, 2).map(([m, n]) => `${n}× ${m}`).join(' · ')
    return { name: 'client_errors', group: 'platform', label: 'App errors (24h)', status: rows.length >= 10 ? 'fail' : 'warn', detail: `${rows.length} report(s), ${byMsg.size} distinct — ${top}` }
  } catch {
    return null
  }
}
...
function buildReport(checks: Check[], ranAt: Date, streak: number): { overall: string; text: string; attention: Check[] } {
  ...
  if (overall !== 'GREEN' && streak > 1) {
    lines.push(`This is night ${streak} in a row that the audit has not been GREEN. The items under "Needs attention" are the same ones each night until somebody acts on them.`)
  }
  lines.push('')
...
// How many consecutive stored runs (most recent first) were not GREEN. The
// current run is counted by the caller.
async function priorNonGreenStreak(admin: any): Promise<number> {
  try {
    const { data } = await admin.from('nightly_audit_runs').select('overall').order('ran_at', { ascending: false }).limit(30)
    let n = 0
    for (const r of data || []) { if (r.overall === 'GREEN') break; n++ }
    return n
  } catch { return 0 }
}

function subjectFor(overall: string, streak: number, attention: Check[]): string {
  const head = overall === 'RED' ? '🔴' : overall === 'AMBER' ? '🟡' : '🟢'
  if (overall === 'GREEN') return `${head} Properly nightly audit — GREEN`
  const first = attention[0] ? ` · ${attention[0].label}` : ''
  const escalate = overall === 'RED' || streak >= 2
  const streakText = streak >= 2 ? ` for ${streak} nights` : ''
  return `${head} ${escalate ? 'ACTION NEEDED · ' : ''}Properly audit ${overall}${streakText}${first}`
}

async function sendEmail(subject: string, text: string, escalate: boolean): Promise<void> {
  if (!RESEND_API_KEY) { console.warn('[nightly-audit] no RESEND_API_KEY — email skipped'); return }
  const to = [AUDIT_EMAIL_TO]
  if (escalate && ESCALATION_TO && ESCALATION_TO !== AUDIT_EMAIL_TO) to.push(ESCALATION_TO)
...
serve(async (req) => {
  ...
  const [web, platform, canary, stripe, resend, clientErrors, priorStreak] = await Promise.all([
    ...
    stripeChecks(admin),
    resendCheck(),
    clientErrorCheck(admin),
    priorNonGreenStreak(admin),
  ])
  checks.push(...web, ...platform, canary, ...stripe, resend)
  if (clientErrors) checks.push(clientErrors)

  // Stable ordering inside each group for a readable digest.
  const order = ['site', 'landing', 'seo_files', 'bundle', 'portals',
    'auth_health', 'rest', 'canary', 'cron', 'http_calls', 'db_checks', 'resend', 'client_errors',
  ...
  const provisional = checks.some(c => c.status === 'fail') ? 'RED' : checks.some(c => c.status === 'warn') ? 'AMBER' : 'GREEN'
  const streak = provisional === 'GREEN' ? 0 : priorStreak + 1
  const { overall, text, attention } = buildReport(checks, ranAt, streak)
  ...
  if (!flags.dry_run) {
    try {
      const escalate = overall === 'RED' || streak >= 2
      await sendEmail(subjectFor(overall, streak, attention), text, escalate)
    } catch (e) { errors.push(`email: ${String(e).slice(0, 100)}`) }
  }

  console.log(`[nightly-audit] ${overall} (streak ${streak}) · ${checks.length} checks · errors: ${errors.join('; ') || 'none'}`)
  return new Response(JSON.stringify({ overall, streak, checks, errors, report: text }), {
